...
async function podAccess(req, res, next) {
  ...
  const r = await db.execute({ sql: 'SELECT user_id FROM pod_members WHERE pod_id=? AND user_id=?', args: [podId, req.user.id] });
...
app.post('/api/auth/signup', async (req, res) => {
  try {
    ...
    await db.execute({ sql: 'INSERT INTO users(name, email, password_hash) VALUES(?,?,?)', args: [name.trim(), email.toLowerCase().trim(), hashPass(password)] });
    const userR = await db.execute({ sql: 'SELECT id, name, email FROM users WHERE email=?', args: [email.toLowerCase()] });
    ...
    // Claim orphaned data
    const orphan = await db.execute('SELECT COUNT(*) as c FROM snapshots WHERE pod_id=0');
    if (orphan.rows[0].c > 0) {
      const mods = await db.execute('SELECT DISTINCT module FROM snapshots WHERE pod_id=0');
      for (const row of mods.rows) {
        await db.execute({ sql: 'INSERT OR IGNORE INTO modules(pod_id, name) VALUES(?,?)', args: [podId, row.module] });
      }
      await db.execute({ sql: 'UPDATE snapshots SET pod_id=? WHERE pod_id=0', args: [podId] });
    }
...
app.post('/api/auth/signin', async (req, res) => {
  try {
    ...
    const r = await db.execute({ sql: 'SELECT id, name, email, password_hash FROM users WHERE email=?', args: [email.toLowerCase()] });
...
app.get('/api/auth/me', auth, async (req, res) => {
  const r = await db.execute({ sql: 'SELECT id, name, email FROM users WHERE id=?', args: [req.user.id] });
...
app.put('/api/pods/:podId', auth, podAccess, async (req, res) => {
  const name = (req.body.name || '').trim();
  if (!name) return res.status(400).json({ error: 'Name required' });
  await db.execute({ sql: 'UPDATE pods SET name=? WHERE id=?', args: [name, req.podId] });
  res.json({ ok: true });
});

app.delete('/api/pods/:podId', auth, podAccess, async (req, res) => {
  const r = await db.execute({ sql: 'SELECT created_by FROM pods WHERE id=?', args: [req.podId] });
  if (r.rows.length === 0 || r.rows[0].created_by !== req.user.id) return res.status(403).json({ error: 'Only owner can delete' });
  await db.execute({ sql: 'DELETE FROM snapshots WHERE pod_id=?', args: [req.podId] });
  await db.execute({ sql: 'DELETE FROM modules WHERE pod_id=?', args: [req.podId] });
  await db.execute({ sql: 'DELETE FROM pod_members WHERE pod_id=?', args: [req.podId] });
  await db.execute({ sql: 'DELETE FROM pods WHERE id=?', args: [req.podId] });
  res.json({ ok: true });
});
...
app.post('/api/pods/:podId/invite', auth, podAccess, async (req, res) => {
  try {
    ...
    const existing = await db.execute({ sql: 'SELECT user_id FROM pod_members WHERE pod_id=? AND user_id=?', args: [req.podId, u.rows[0].id] });
    if (existing.rows.length > 0) return res.status(400).json({ error: 'User is already a member' });
    await db.execute({ sql: 'INSERT INTO pod_members(pod_id, user_id, role) VALUES(?,?,?)', args: [req.podId, u.rows[0].id, 'member'] });
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.delete('/api/pods/:podId/members/:userId', auth, podAccess, async (req, res) => {
  ...
  await db.execute({ sql: 'DELETE FROM pod_members WHERE pod_id=? AND user_id=?', args: [req.podId, userId] });
  res.json({ ok: true });
});
...
app.get('/api/pods/:podId/modules', auth, podAccess, async (req, res) => {
  const r = await db.execute({ sql: 'SELECT name FROM modules WHERE pod_id=? ORDER BY name', args: [req.podId] });
...
app.post('/api/pods/:podId/modules', auth, podAccess, async (req, res) => {
  const name = (req.body.name || '').trim();
  if (!name) return res.status(400).json({ error: 'Name required' });
  await db.execute({ sql: 'INSERT OR IGNORE INTO modules(pod_id, name) VALUES(?,?)', args: [req.podId, name] });
  res.json({ ok: true });
});
...
app.put('/api/pods/:podId/modules/:name', auth, podAccess, async (req, res) => {
  ...
  await db.execute({ sql: 'UPDATE modules SET name=? WHERE pod_id=? AND name=?', args: [newName, req.podId, req.params.name] });
  await db.execute({ sql: 'UPDATE snapshots SET module=? WHERE pod_id=? AND module=?', args: [newName, req.podId, req.params.name] });
  res.json({ ok: true });
});

app.delete('/api/pods/:podId/modules/:name', auth, podAccess, async (req, res) => {
  await db.execute({ sql: 'DELETE FROM snapshots WHERE pod_id=? AND module=?', args: [req.podId, req.params.name] });
  await db.execute({ sql: 'DELETE FROM modules WHERE pod_id=? AND name=?', args: [req.podId, req.params.name] });
  res.json({ ok: true });
});
...
app.get('/api/pods/:podId/data/:module/dates', auth, podAccess, async (req, res) => {
  const r = await db.execute({ sql: 'SELECT date FROM snapshots WHERE pod_id=? AND module=? ORDER BY date DESC', args: [req.podId, req.params.module] });
...
app.get('/api/pods/:podId/data/:module/:date', auth, podAccess, async (req, res) => {
  ...
  const exact = await db.execute({ sql: 'SELECT root_title, data FROM snapshots WHERE pod_id=? AND module=? AND date=?', args: [req.podId, module, date] });
  if (exact.rows.length > 0) {
    const r = exact.rows[0];
    return res.json({ rootTitle: r.root_title, modules: JSON.parse(r.data), _saved: true, _from: null });
  }
  const prev = await db.execute({ sql: 'SELECT root_title, data, date FROM snapshots WHERE pod_id=? AND module=? AND date<=? ORDER BY date DESC LIMIT 1', args: [req.podId, module, date] });
...
app.post('/api/pods/:podId/data/:module/:date', auth, podAccess, async (req, res) => {
  ...
  await db.execute({ sql: 'INSERT OR IGNORE INTO modules(pod_id, name) VALUES(?,?)', args: [req.podId, req.params.module] });
  await db.execute({
...
app.delete('/api/pods/:podId/data/:module/:date', auth, podAccess, async (req, res) => {
  await db.execute({ sql: 'DELETE FROM snapshots WHERE pod_id=? AND module=? AND date=?', args: [req.podId, req.params.module, req.params.date] });
  res.json({ ok: true });
});
